...
The project owner is the user (a student). Prefer working in code: create,
debug, iterate. Machine: MacBook Air M5; no M-series-specific app code.
...
- Remote Git and GitHub operations are authorized **only** for this project and
  only when the target is `iktCalT/climate2`. Before any network operation,
  verify that `origin` resolves exactly to that repository. Never push, pull,
  fetch, open a pull request against, merge, rename, or remove a remote for any
  other repository. Do not use another local remote alias when `origin` is
  available, and never contact the original `iktCalT/climate` repository.
- For each completed change: work and commit on a `codex/<short-description>`
  branch, push that branch to `origin`, open a GitHub pull request targeting
  `main`, and use **rebase and merge**. Then switch to local `main` and pull the
  merged result before creating the next branch. Do not force-push or push
  directly to `main`.
- Never implement changes on `main`. Before a substantial change, create or
  switch to a non-main branch (prefer `codex/<short-description>`), then commit
  the tested change locally on that branch.
...
- If a tracked file is found to contain a real secret or personal data, stop,
  ...
  the user before removing it or rewriting Git history.
